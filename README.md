...
`Context Budget` 규칙이 글이 아닌 측정값으로 검증되도록 stderr 로깅 도입. 이후 모든 변경의 토큰 영향이 가시화되어 "토큰을 더 쓸 곳"과 "줄일 곳"을 데이터 기반으로 결정 가능.

- **`hooks/extract-constraints.sh` stderr 로깅** — agent별 추출 출력의 바이트 수 + 근사 토큰 수(`bytes/4`) 출력. `[token-budget] extract-constraints reviewer: 3158 bytes (~789 tokens)`. stdout(agent 컨텍스트)은 변경 없음
- **`hooks/post-tool-use.sh` ERRORS 블록 측정** — 검증 실패 피드백 크기를 stderr로 로깅. 다음 턴 컨텍스트 비대화 사전 감지
- **8KB 경고 임계값** — 초과 시 `[warn]` 태그 출력. `FE_COPILOT_BUDGET_WARN_BYTES` 환경변수로 오버라이드 가능
- **`CLAUDE.md → Budget Measurement` 섹션 신설** — 임계 초과 시 처리 가이드(섹션 가지치기 또는 태그 분할) 명시
- **현재 측정값** — reviewer 3158 bytes (~789 tokens), developer/ui-builder/api-integrator 1328 bytes (~332 tokens). 모두 예산 내. 8KB 임계 초과 발생 시 즉시 가시화
- **토큰 영향 ≈ 0** — stderr 전용, agent 컨텍스트 무영향. 런타임 비용 무시 수준 (`wc -c` 1회)
...
- [x] **토큰 예산 측정 인프라 v6.9.3** — `hooks/extract-constraints.sh` + `hooks/post-tool-use.sh` stderr 로깅, 8KB 경고 임계, `FE_COPILOT_BUDGET_WARN_BYTES` 오버라이드, `CLAUDE.md → Budget Measurement` 가이드
