...
- [ ] **v6.9.2 — failure-patterns 카테고리 + refactor 게이트 누수 차단** (push #1)
  - `constraints.md #failure-patterns` 형식 확장: `- [YYYY-MM-DD] [category:design|correctness|react-perf|scope] [pattern] - [rule]`
  - `agents/refactor-architect.md` 책임 명시: 아키텍처 변경 후 `plan.md → Design Check`(특히 Reuse/Risk)을 in-place 갱신해야 developer가 일관된 입력 수신
  - 토큰 영향: +5줄. 누적되는 design 패턴이 자동 차단 룰로 진화하는 학습 루프 완성
